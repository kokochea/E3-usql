...
def p_query_select(p):
    '''query : SELECT seleccion FROM tabla opcionales'''
    p[0] = f"TRAEME {p[2]} DE LA TABLA {p[4]} {p[5]}"
...
def p_seleccion(p):
    ...
    elif p[1] == "CONTANDO" and p[3] == '*':
        p[0] = 'COUNT(*)'
    elif p[1] == "CONTANDO" and p[3] == "TODO":
        p[0] = 'COUNT(*)'
    else:
        p[0] = p[1]
...
if __name__ == '__main__':
    ...
    # lexer.input('TRAEME CONTANDO(TODO) DE LA TABLA ventas AGRUPANDO POR producto WHERE DEL GROUP BY CONTANDO(*) > 5')
    # while True:
    #     tok = lexer.token()
    #     if not tok:
    #         break      # No more input
